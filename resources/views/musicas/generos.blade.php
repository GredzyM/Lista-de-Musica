...
<body>
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-8">
            <h1 class="text-3xl font-bold mb-4" style="color: var(--color-text);">🎵 Géneros Musicales</h1>
            <p class="text-gray-600">Explora nuestra colección de {{ count($generos) }} géneros diferentes</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach($generos as $genero)
                @php
                    $generoCanciones = $musicas->where('genero', $genero);
                    $primeraCancion = $generoCanciones->first();
                @endphp
                <div class="genre-card bg-white rounded-lg overflow-hidden">
                    <div class="image-container">
                        ...
                        <div class="absolute inset-0 bg-black bg-opacity-40 flex items-end">
                            <div class="p-4 w-full">
                                <h2 class="text-white text-xl font-bold truncate">{{ $genero }}</h2>
                                <p class="text-white text-sm opacity-80">{{ $generoCanciones->count() }} canciones</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-4">
                        <div class="space-y-2">
                            @foreach($generoCanciones->take(3) as $cancion)
                                <div class="flex items-center justify-between">
                                    <span class="text-sm truncate flex-1">{{ $cancion->titulo }}</span>
                                    <span class="text-xs text-gray-500">{{ $cancion->artista }}</span>
                                </div>
                            @endforeach
                        </div>
                        <div class="mt-4 text-center">
                            <a href="#" class="inline-block px-4 py-2 rounded-full text-sm hover:bg-opacity-80" 
